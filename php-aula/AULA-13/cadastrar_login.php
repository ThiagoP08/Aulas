<?php
    ...
    if($name && $email && $password && $password_confirm){  
        ...
        $sql = $pdo->prepare("select * from usuario where email = :email");    //Seleciona os valores da tabela usuario onde email possui um valor
        ...
        if($sql->rowCount() === 0){
            ...
            if($password === $password_confirm){    //Verifica se a "Senha" e "Confirmar Senha" são iguais, se forem, as funções são executadas
                ...
                $sql = $pdo->prepare( "INSERT INTO usuario (nome,email,senha) VALUES (:name, :email, :password)" );    //Adiciona os valores na tabla "usuario"
